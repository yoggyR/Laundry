<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-2 border-bottom">
    <label><span style="font-weight: bold;"> <?php echo ($pages); ?> </span></label>
</div>
<div class='row'>
    <div class="md-col-12">
        <div class="countainer count_data_report">
            <div class="row">
                <div class="col-1 receptacle_data_report" style="margin-left: 38px;">
                    <label class="label_data_report shadow">
                        <p class="txt_head_data_report">On process</p>
                        <h1 class="txt_real_data_report">03</h1>
                        <img class="img_data_report" src="../../ASSET/ICON/PACKAGE/7097271_washing_bubble_detergent powder_t-shirt_laundry_icon.svg" alt="">
                        <p class="txt_powered_by_report">Powered by Laundry&trade;</p>
                    </label>
                </div>
                <div class="col-1 receptacle_data_report">
                    <label class="label_data_report shadow">
                        <p class="txt_head_data_report">Finished</p>
                        <h1 class="txt_real_data_report">01</h1>
                        <img class="img_data_report" src="../../ASSET/ICON/PACKAGE/7097281_washing_folding_clothes_basket_laundry_icon.svg" alt="">
                        <p class="txt_powered_by_report">Powered by Laundry&trade;</p>
                    </label>
                </div>
                <div class="col-1 receptacle_data_report">
                    <label class="label_data_report shadow">
                        <p class="txt_head_data_report">Taken</p>
                        <h1 class="txt_real_data_report">01</h1>
                        <img class="img_data_report" src="../../ASSET/ICON/PACKAGE/7011187_hand_person_clothes_over_people_icon.svg" alt="">
                        <p class="txt_powered_by_report">Powered by Laundry&trade;</p>
                    </label>
                </div>
                <div class="col-1 receptacle_data_report">
                    <label class="label_data_report shadow">
                        <p class="txt_head_data_report">Customers</p>
                        <h1 class="txt_real_data_report">05</h1>
                        <img class="img_data_report" src="../../ASSET/ICON/PACKAGE/3209291_contacts_customers_family_group_team_icon.svg" alt="">
                        <p class="txt_powered_by_report">Powered by Laundry&trade;</p>
                    </label>
                </div>
                <div class="col-1">
                    <label class="label_data_report shadow">
                        <p class="txt_head_data_report">Profit</p>
                        <h1 class="txt_real_data_report_profit">IDR 201.000</h1>
                        <img class="img_data_report_profit" src="../../ASSET/ICON/PACKAGE/290143_cash_money_payment_wallet_icon.svg" alt="">
                        <p class="txt_powered_by_report_profit">Powered by Laundry&trade;</p>
                    </label>
                </div>
            </div>
            <div class="shadow receptacle_data_table_report">
                <div class="row">
                    <div class="col-6">
                        <form class="d-flex form_search_data_table_reoprt">
                            <input class="form-control me-2 input_search_data_table_report" type="search" placeholder="Search data in the table" aria-label="Search">
                            <button class="btn btn_search_data_table_report" type="submit">Search</button>
                        </form>
                    </div>
                    <div class="col-6">
                        <button class="btn btn_export_to_excel" type="button">Export to excel<img src="../../ASSET/ICON/PACKAGE/7422413_exel_spreadsheet_sheets_table_icon.svg" class="img_export_to_excel"></button>
                    </div>
                </div>
                <table class="table table-bordered table_report table-hover">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Customers name</th>
                            <th>Customers id</th>
                            <th>Package name</th>
                            <th>Level</th>
                            <th>Weight</th>
                            <th>Order</th>
                            <th>Finished process</th>
                            <th>Taken</th>
                            <th>Price already paid</th>
                            <th>Unpaid price</th>
                            <th>Price to pay</th>
                            <th>Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>
                                <p class="txt_data_table_report">1</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">Roger</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">LNY-184626</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">Regular</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">1</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">10 Kg</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">14-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">18-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">18-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 70.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR -</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 70.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report txt_data_status_taken">Taken</p>
                            </td>
                        </tr>
                        <!-- ============== -->
                        <tr>
                            <td>
                                <p class="txt_data_table_report">2</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">David</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">LNY-175629</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">Regular</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">1</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">5 Kg</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">19-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">23-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">-</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 10.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 25.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 35.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report txt_data_status_finished">Finished</p>
                            </td>
                        </tr>
                        <!-- ============== -->
                        <tr>
                            <td>
                                <p class="txt_data_table_report">3</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">Alex</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">LNY-385760</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">Regular</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">1</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">8 Kg</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">23-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">27-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">-</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 56.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR -</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 56.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report txt_data_status_on_process">On process</p>
                            </td>
                        </tr>
                        <!-- ============== -->
                        <tr>
                            <td>
                                <p class="txt_data_table_report">4</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">Brian</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">LNY-576925</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">Regular</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">2</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">6 Kg</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">23-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">28-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">-</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 15.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 45.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 60.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report txt_data_status_on_process">On process</p>
                            </td>
                        </tr>
                        <!-- ============== -->
                        <tr>
                            <td>
                                <p class="txt_data_table_report">5</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">James</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">LNY-275610</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">Regular</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">1</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">20 Kg</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">23-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">27-10-2021</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">-</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 50.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 90.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report">IDR 140.000</p>
                            </td>
                            <td>
                                <p class="txt_data_table_report txt_data_status_on_process">On process</p>
                            </td>
                        </tr>
                        <!-- ============== -->

                    </tbody>
                </table>

                <div class="btn-toolbar justify-content-end" role="toolbar" aria-label="Toolbar with button groups">
                    <div class="btn-group receptacle_btn_next_or_back_report" role="group" aria-label="Second group">
                        <button type="button" class="btn extra_next_or_back_report btn_next_or_back_report" disabled><img src="../../ASSET/ICON/PACKAGE/352072_arrow_play_icon - Edit.svg" class="arrow_next_and_back_report"> Back
                        </button>

                        <button type="button" class="btn extra_next_or_back_report" style="background-color: gray; color: white;">1</button>
                        <button type="button" class="btn extra_next_or_back_report">2</button>
                        <button type="button" class="btn extra_next_or_back_report">3</button>

                        <button type="button" class="btn extra_next_or_back_report btn_next_or_back_report">Next <img src="../../ASSET/ICON/PACKAGE/352072_arrow_play_icon.svg" class="arrow_next_and_back_report">
                        </button>
                    </div>
                </div>

            </div>
        </div>
    </div>
</div>